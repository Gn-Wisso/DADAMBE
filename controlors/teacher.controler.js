...
function sendPassword(firstName, last_name, password, email) {
  ...
  sendM.sendEmail(email, subject, {
    ...
    Message:
      "Your account user in our Platform as a Teacher  has been created. Please Log in by this password : \n" +
      password +
      " .",
    Url: null,
  });
...
const addTeacher = async (req, res, next) => {
  try {
    ...
    let generatedCode = generateStudentCode(
      reqData.firstName,
      reqData.lastName,
      reqData.dateOfBirth,
      reqData.email
  ...
  try {
    ...
    for (const teacher of teachers) {
      if (
        ...
        teacher.personProfile2.imagePath != ""
      ) {
        ...
        try {
          ...
          teacher.personProfile2.imagePath = await fs.promises.readFile(
            photoPath
...
const ExploreSearch = async (req, res, next) => {
  try {
    ...
    const options = {
      includeScore: true,
      keys: [
        "personProfile2.firstName",
        "personProfile2.lastName",
        "personProfile2.mail",
        "personProfile2.phoneNumber",
        "subject",
      ],
    };
...
const getTeacherData = async (req, res, next) => {
  try {
    ...
    if (userTeacher.personProfile2.imagePath) {
      const photoPath = path.join(
        "uploads/profileImage/",
        userTeacher.personProfile2.imagePath
      );
      try {
        await fs.promises.access(photoPath, fs.constants.F_OK);
        userTeacher.personProfile2.imagePath = await fs.promises.readFile(
          photoPath
        ); // read the photo file contents
      } catch (error) {
        console.error(error);
