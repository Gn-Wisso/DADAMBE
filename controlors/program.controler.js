  ...
  try {
    ...
    if (!isSkip) {
      if (type == "formation") {
        ...
        const { inscriptionEndDay, nbrSession, hoursBySession } =
          req.body.dataType;
        ...
        const { timing, emplacement, inscriptionEndDay } = req.body.dataType;
...
const updateProgram = async (req, res, next) => {
  try {
    ...
    return res.send({
      ...
      programId: programData.ID_ROWID,
      code: 200,
    });
  } catch (error) {
    console.log(error);
